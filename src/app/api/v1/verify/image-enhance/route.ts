...
// ─── Shared types (mirror client-side) ─────────────────────────────────
...
/** Map a -100..100 brightness to sharp's modulate factor (1.0 = no change). */
function brightnessToFactor(b: number): number {
  const clamped = Math.max(-100, Math.min(100, b));
  // -100 → 0.5, 0 → 1.0, +100 → 1.5
  return 1.0 + (clamped / 100) * 0.5;
}

/** Map a -100..100 contrast to sharp's modulate factor. */
function contrastToFactor(c: number): number {
  const clamped = Math.max(-100, Math.min(100, c));
  // -100 → 0.5, 0 → 1.0, +100 → 1.5
  return 1.0 + (clamped / 100) * 0.5;
}

/** Build the sharp pipeline from the requested options. */
async function buildEnhancedPipeline(
  buffer: Buffer,
  options: EnhanceOptions
): Promise<sharp.Sharp> {
  let pipeline = sharp(buffer, { failOn: "none" }).rotate(); // auto-EXIF

  // Resize first if a target width is given
  ...
  // Normalize = auto-contrast stretch (linear histogram stretch)
  // when autoCrop is enabled we also call normalize for safety.
  if (options.autoCrop) {
    pipeline = pipeline.normalize();
  }

  // Brightness/contrast via modulate
  const modulate: any = {};
  if (options.brightness !== undefined) {
    modulate.brightness = brightnessToFactor(options.brightness);
  }
  if (options.contrast !== undefined) {
    modulate.contrast = contrastToFactor(options.contrast);
  }
  if (Object.keys(modulate).length > 0) {
    pipeline = pipeline.modulate(modulate);
  }

  // Median denoise (3x3) — sharp's median() takes a size, default 3 = 3x3
  if (options.denoise) {
    pipeline = pipeline.median(3);
  }

  // Unsharp mask — sharp's sharpen() takes { sigma, m1, m2, x1, y2, y3 }
  if (options.sharpness !== undefined && options.sharpness > 0) {
    // Map 0..100 to a sigma between 0.5 (mild) and 2.5 (aggressive)
    const sigma = 0.5 + (Math.min(100, options.sharpness) / 100) * 2.0;
    pipeline = pipeline.sharpen({
      sigma,
      // m1/m2 control the "flat" and "jagged" thresholds — defaults work
      // well for documents; we just scale them slightly.
      m1: 1.0,
      m2: 2.0,
    });
  }
...
// ─── Quality analysis (server-side) — copied from image-analysis ────
//
// This is duplicated here to keep the image-enhance route self-contained
// (no internal cross-route dependency). The two copies must stay in
// sync — see also: src/app/api/v1/verify/image-analysis/route.ts
...
/**
 * POST /api/v1/verify/image-enhance
 *
 * Server-side image enhancement. Applies brightness/contrast/sharpness/
 * denoise/auto-contrast to an uploaded image and returns:
 *   - The enhanced image (base64 JPEG data URL)
 *   - The list of adjustments applied
 *   - The post-enhancement ImageQualityReport
 *
 * Body: {
 *   image: string (base64 data URL or raw),
 *   options?: {
 *     brightness?: number,   // -100..100
 *     contrast?: number,     // -100..100
 *     sharpness?: number,    // 0..100
 *     denoise?: boolean,
 *     autoCrop?: boolean,    // applies sharp's normalize (auto-contrast stretch)
 *     targetWidth?: number,  // resize to this width (preserves aspect)
 *   },
 * }
 *
 * Returns: { image: string, adjustments, quality, durationMs }
 */
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const options: EnhanceOptions = body?.options || {};
    const docType: DocType = (
      ["national_id", "passport", "driver_license"].includes(body?.docType as DocType)
    ...
    const pipeline = await buildEnhancedPipeline(buffer, options);

    // Encode as JPEG (flatten alpha if present, white bg)
    const meta = await sharp(buffer, { failOn: "none" }).metadata();
    const hasAlpha = meta.hasAlpha || meta.channels === 4;
    const finalPipeline = hasAlpha
      ? pipeline.flatten({ background: { r: 255, g: 255, b: 255 } }).jpeg({ quality: 85, mozjpeg: true })
      : pipeline.jpeg({ quality: 85, mozjpeg: true });
    const outBuffer = await finalPipeline.toBuffer();

    const outBase64 = outBuffer.toString("base64");
    const outDataUrl = `data:image/jpeg;base64,${outBase64}`;

    // Run the same quality analysis on the enhanced output so callers
    // can compare before/after.
    const quality = await analyzeQuality(outBuffer, docType);
    ...
    const adjustments = {
      ...
      autoContrast: !!options.autoCrop,
      targetWidth: options.targetWidth ?? null,
      exifRotated: true, // pipeline called .rotate()
    ...
    return NextResponse.json(
      {
        error: e?.message || "image enhancement failed",
        code: "IMAGE_ENHANCE_FAILURE",
        durationMs: Date.now() - startedAt,
      },
...
/**
 * GET /api/v1/verify/image-enhance
 * API documentation + the adjustment scales.
 */
export async function GET() {
  return NextResponse.json(
    {
      description:
        "Server-side image enhancement — applies brightness/contrast/sharpness/denoise/auto-contrast and returns the enhanced image plus a post-enhancement quality report.",
      method: "POST",
      body: {
        image: "base64-encoded JPEG/PNG (data URL or raw)",
        options: {
          brightness: "number -100..100 (sharp.modulate brightness factor 0.5..1.5)",
          contrast: "number -100..100 (sharp.modulate contrast factor 0.5..1.5)",
          sharpness: "number 0..100 → sharp.sharpen sigma 0.5..2.5",
          denoise: "boolean → sharp.median(3) 3x3 median filter",
          autoCrop: "boolean → sharp.normalize() auto-contrast histogram stretch",
          targetWidth: "number → sharp.resize(width, fit: inside, withoutEnlargement: true)",
        },
        docType: "national_id | passport | driver_license (default: national_id) — used for DPI estimate in quality report",
      },
      response: {
        image: "data:image/jpeg;base64,...",
        adjustments: "summary of all adjustments applied",
        quality: "ImageQualityReport computed on the enhanced output",
        durationMs: "processing time in milliseconds",
      },
      notes: [
        "All enhancements run server-side via sharp — no client canvas required.",
        "EXIF orientation is auto-applied (sharp.rotate()) before any other operation.",
        "Output is always JPEG (flattened onto white if source had alpha) at quality 85.",
        "The quality report uses the same thresholds as the client-side analyzeImageQuality().",
      ],
    },
    { headers: CORS_HEADERS }
  );
